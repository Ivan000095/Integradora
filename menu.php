<nav class="navbar navbar-expand-lg ">
    <div class="container">
        <a class="navbar-brand" href="#">
            <img src="#" alt="Logo" width="50" height="50" class="d-inline-block align-text-top">
        </a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                <li class="nav-item"><a class="nav-link" href="/Servifinder Oficial/vista/">Inicio</a></li>
                <li class="nav-item"><a class="nav-link" href="/Servifinder Oficial/">¿Quienes somos?</a></li>
                <li class="nav-item"><a class="nav-link" href="/Servifinder Oficial/footer.php/">Contacto</a></li>
                <li class="nav-item"><a class="nav-link" href="/Servifinder Oficial/vista/Comentario/">Comentarios</a></li>
                <li class="nav-item dropdown">
                    <a href="" class="nav-link">Servicios</a>
                    <ul class="dropdown-menu">
                        <li><a href="/ServiFinder Oficial/vista/cliente/doctor/">Médicos</a></li>
                        <li><a href="/ServiFinder Oficial/vista/cliente/profesionista/">Técnicos</a></li>
                        <li><a href="/ServiFinder Oficial/vista/cliente/establecimiento/">Otros</a></li>
                    </ul>
                </li>
                <li class="Registro"><a class="nav-link" href="/Servifinder Oficial/vista/login/"><ion-icon name="person-outline"></ion-icon>  Registrarse/Iniciar sesión</i></a>
            </ul>
        </div>
    </div>
</nav>
