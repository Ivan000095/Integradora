...
class Doctor {
    ...
    public function save() {
        ...
        if( $this->idDoctor==0 ) {
           try{
            ...
            $sql1 = $con->prepare("INSERT INTO TUsuarios(correo,password) VALUES (?,?)");
            $sql1->bind_param("ss", $this->correo, $this->password);
            $sql1->execute();
            ...
            try{
            $con->begin_transaction();
            $sql1 = $con->prepare("UPDATE TUsuario SET correo = ?, password = ? WHERE  Id_Usuario = ?");
            $sql1->bind_param("ss", $this->correo, $this->password,$this->idusr);
            $sql1->execute();
            ...
            $sql3->bind_param("idssssi",$this->idespecialidad,$this->costo,$this->horario,$this->horario,$this->diaslab,$this->ubicacion,$this->idDoctor);
            $sql3->execute();
    ...
    public function saveres($id){
        include('conexion.php');
        $sql="INSERT INTO ReseñaDoc(Puntuacion, Comentario, NombreUsr, Id_Doctor) VALUES(" . $this->puntuacion . ", '" . $this->comentario . "', '" .$this->nombreusr . "', $idDoctor)";
        return $con->query($sql);
    }

    public static function findres($id) {
        include('conexion.php');
        $sql="SELECT * FROM ReseñaDoc WHERE Id_Doctor= " . $idDoctor;
        return $con->query($sql);
    }
